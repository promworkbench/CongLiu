...
public class VisualizeHPNandInteraction2Dot {
	...
	public static DotCluster visualizeHPN2Dot(HierarchicalPetriNet hpn, String currentName, final DotCluster dot, HashMap<XEventClass, DotCluster> xeventClass2InteractionCluster)
	{	
		...
		for (final Transition t : pn.getTransitions()) 
		{
			...
			if(xeventClass2InteractionCluster!=null)
			{
				// check if the current transition is a interaction activity
				for(XEventClass xEventClass:xeventClass2InteractionCluster.keySet())
				{
					if(t.getLabel().equals(xEventClass.toString()))
					{
						//add an edge from the current t to the cluster
						LocalDotEdge tempEdge = new LocalDotEdge(tDot, xeventClass2InteractionCluster.get(xEventClass), 1);
						dot.addEdge(tempEdge);
					
						// set arc from the nested transition to cluster
						tempEdge.setOption("lhead", xeventClass2InteractionCluster.get(xEventClass).getId());
...
//			pDot = new LocalDotPlace();
		...
		if(XEventClass2hpn.keySet().size()!=0)
		{
			for(XEventClass xevent: XEventClass2hpn.keySet())
			{
				...
				visualizeHPN2Dot(XEventClass2hpn.get(xevent),ActivityNameMethodClassPackage(xevent.toString()), Nested_cluster, xeventClass2InteractionCluster);// recursion, c2Inter
