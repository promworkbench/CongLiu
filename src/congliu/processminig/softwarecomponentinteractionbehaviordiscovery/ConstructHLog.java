...
/**
 * this class extends the ConstructHSoftwareEventLog in 
 * congliu.processmining.softwareprocessmining.InitializeSoftwareEventLog package.
...
public class ConstructHLog {
	 ...
	 * @return
	 */
	public static HSoftwareEventLog ConstructHLogRecusively(XFactory factory, ArrayList<Component2Classes> c2c, XEventClass xeventclass, 
			HashSet<XEvent> eventList, XLog originalLog, XEventClassifier currentClassifier, String ComponentName)
	{
		...
		{
			if (com2Class.getComponent().equals(ComponentName))
			{
				classSet = com2Class.getClasses();
			}
		}
		...
		if ((xeventclass==null) && (eventList==null))// means this is the top-level.
		{
			...
			if (xeventclass2Eventlist.size()>0)// there exist nested events
			{
				...
				{
					subLogMapping.put(xeventc, ConstructHLogRecusively(factory,c2c, xeventc, xeventclass2Eventlist.get(xeventc), 
...
//			}
	...
	public static MainLogAndNestedEventListClass getMainOtherLevels(XLog mainLog, HashSet<XEvent> eventList, XLog originalLog, XFactory factory, String componentName)
	{
		...
		// we need to separately cope with different caller, main and others. 
			...
			for (XTrace trace:originalLog)
			{
				...
				for (XEvent calleeEvent: trace)
				{
					if(XSoftwareExtension.instance().extractClassObject(callerEvent).equals(XSoftwareExtension.instance().extractCallerclassobject(calleeEvent))
							...
							//&&XSoftwareExtension.instance().extractClass(callerEvent).equals(XSoftwareExtension.instance().extractCallerclass(calleeEvent))
							//&&XSoftwareExtension.instance().extractPackage(callerEvent).equals(XSoftwareExtension.instance().extractCallerpackage(calleeEvent))
							)
					{
