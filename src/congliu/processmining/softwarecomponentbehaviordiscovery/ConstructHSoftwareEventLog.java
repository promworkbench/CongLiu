...
public class ConstructHSoftwareEventLog {
	...
	public static HSoftwareEventLog ConstructHSoftwareEventLogRecusively(XEventClass xeventclass, HashSet<XEvent> eventList, XLog originalLog, XEventClassifier currentClassifier)
	{
		...
		if ((xeventclass==null) && (eventList==null))// means this is the top-level.
		{
			...
			mainLogNestedList = getStartMain(mainLog, originalLog, factory);
...
//			}
	...
	public static MainLogAndNestedEventListClass getMainOtherLevels(XLog mainLog, HashSet<XEvent> eventList, XLog originalLog, XFactory factory)
	{
		...
		for(XEvent callerEvent: eventList)
		{
			...
			for (XTrace trace:originalLog)
			{
				...
				{
					if (XSoftwareExtension.instance().extractClassObject(callerEvent).toString().equals(XSoftwareExtension.instance().extractCallerclassobject(calleeEvent).toString())
							&& XConceptExtension.instance().extractName(callerEvent).toString().equals(XSoftwareExtension.instance().extractCallermethod(calleeEvent).toString()))
					{
						...
						if (nestedFlag.equals("1"))
						{
							...
							XSoftwareExtension.instance().assignNesting(calleeEvent, true);
						}
						else
						{
							//if it is not nested, add the nested_flag ==0.
							//calleeEvent.getAttributes().put("Nested_Flag", new XAttributeLiteralImpl("Nested_Flag",nestedFlag));
							XSoftwareExtension.instance().assignNesting(calleeEvent, false);
						}
	...
	public static MainLogAndNestedEventListClass getStartMain(XLog mainLog, XLog originalLog, XFactory factory)
	{
	...
	public static String checkNesting(XEvent event, XTrace trace)
	{
		...
		for (XEvent e: trace)
		{	
			if (XSoftwareExtension.instance().extractClassObject(event).toString().equals(XSoftwareExtension.instance().extractCallerclassobject(e).toString())
					&& XConceptExtension.instance().extractName(event).toString().equals(XSoftwareExtension.instance().extractCallermethod(e).toString()))
			{
				nestedFlag="1";
				return nestedFlag;
			}
