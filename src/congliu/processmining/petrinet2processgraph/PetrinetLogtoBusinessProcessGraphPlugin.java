 ...
 */
@Plugin(
		name = "Transform a Petri net and Log to a Business Process Graph",// plugin name
		...
		userAccessible = true,
		help = "This plugin aims to transform a Petri net together with the log to a business process graph." 
		)

public class PetrinetLogtoBusinessProcessGraphPlugin {
		@UITopiaVariant(
		        affiliation = "TU/e", 
		        author = "Cong liu", 
		...
		public BusinessProcessGraph LogBusinessProcessGraphTransformation(UIPluginContext context, XLog log, Petrinet pn)
		{
			...
			for(Edge e: edgeSet.keySet())
			{
				...
				for(XTrace trace: log)
				{
					...
					{
						if(e.source.equals(XConceptExtension.instance().extractName(trace.get(i))))
						{
							for(int j =i+1;j<trace.size();j++)
							{
								if(e.target.equals(XConceptExtension.instance().extractName(trace.get(j))))
								{
									count++;
									break;
								}
			...
			for(Edge ed: edgeSet.keySet())
			{
				...
				//bpg.setEdgeWeight(edge, edgeSet.get(ed)/log.size());
				bpg.setEdgeWeight(edge, (double)(Math.round(edgeSet.get(ed)/log.size()*100)/100.0));//keep two digit 
