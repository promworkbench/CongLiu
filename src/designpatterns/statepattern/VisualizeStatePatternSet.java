 ...
 *
 */
public class VisualizeStatePatternSet {
	...
	@Plugin(name = "Visualize State Pattern Instances", 
			...
			@PluginVariant(variantLabel = "Visualize State Pattern Instances", 
					requiredParameterLabels = {0})// it needs one input parameter
	
	public JComponent visualizeTop(PluginContext context, StatePatternSet patternSet)
	{
		...
		{
			Dot dot = convertInstance2DOT((StatePatternClass)instance);
	...
	public static Dot convertInstance2DOT(StatePatternClass pattern)
	{
		Dot dot = new Dot();
		...
		String state ="State Class: "+pattern.getState();
		String setState ="setState Method: " +pattern.getSetState();
		String request = "Request Method: "+pattern.getRequest();
		String handle = "Handle Method: "+pattern.getHandle();
		
		
		String traceInvocation= "Trace/Invocation Count: "+pattern.getTraceNumber()+"/"+pattern.getInvocationNumber();
		
		
		DotNode tab = dot.addNode("{"+context+"}|"+"{"+state+"}|"+"{"+setState+"}|"+"{"+request+"}|"+"{"+handle+"}|"+"{"+traceInvocation+"}");
