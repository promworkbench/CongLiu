...
/**
 * this visualizer tries to visualize the specification of each detected pattern instance. 
 * @author cliu3
 *
 */

public class VisualizeCommandPatternSet {
	...
	@Plugin(name = "Visualize Command Pattern Instances", 
			...
			@PluginVariant(variantLabel = "Visualize COmmand Pattern Instances", 
					requiredParameterLabels = {0})// it needs one input parameter
	
	public JComponent visualizeTop(PluginContext context, CommandPatternSet patternSet)
	{
		...
		{
			Dot dot = convertInstance2DOT((CommandPatternClass)instance);
	...
	public static Dot convertInstance2DOT(CommandPatternClass pattern)
	{
		Dot dot = new Dot();
		dot.setDirection(GraphDirection.leftRight);

		// prepare the table with different roles 
		String invoker= "Invoker Class: "+pattern.getInvoker();
		String command ="Command Class: "+pattern.getCommand();
		String receiver ="Receiver Class: "+pattern.getReceiver();
		String call ="call Method: " +pattern.getCall();
		String execute = "execute Method: "+pattern.getExecute();
		String action = "action Method: "+pattern.getAction();
		
		
		String traceInvocation= "Trace/Invocation Count: "+pattern.getTraceNumber()+"/"+pattern.getInvocationNumber();
		
		
		DotNode tab = dot.addNode("{"+invoker+"}|"+"{"+command+"}|"+"{"+receiver+"}|"+"{"+call+"}|"+"{"+execute+"}|"+"{"+action+"}|"+"{"+traceInvocation+"}");
