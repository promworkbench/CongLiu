...
/**
 * this visualizer tries to visualize each detected pattern instance. 
 * @author cliu3
 *
 */
public class VisualizeStrategyPatternSet {
	...
	@Plugin(name = "Visualize Visitor Pattern Instances", 
			...
			@PluginVariant(variantLabel = "Visualize Strategy Pattern Instances", 
					requiredParameterLabels = {0})// it needs one input parameter
	public JComponent visualizeTop(PluginContext context, StrategyPatternSet patternSet)
	{
		...
		{
			Dot dot = convertInstance2DOT((StrategyPatternClass)instance);
	...
	public static Dot convertInstance2DOT(StrategyPatternClass pattern)
	{
		Dot dot = new Dot();
		dot.setDirection(GraphDirection.leftRight);

		// prepare the table
		String context= "Context Class: "+pattern.getContext();
		String strategy ="Strategy Class: "+pattern.getStrategy();
		String contextI= "ContextInterface Method: "+pattern.getContextInterface();
		String algorithmI = "AlgorithmInterface Method: "+pattern.getAlgorithmInterface();
		String setStrategy ="setStrategy Method: " +pattern.getSetStrategy();
		
		
		String traceInvocation= "Trace/Invocation Count: "+pattern.getTraceNumber()+"/"+pattern.getInvocationNumber();
		
		
		DotNode tab = dot.addNode("{"+context+"}|"+"{"+strategy+"}|"+"{"+setStrategy+"}|"+"{"+contextI+"}|"+"{"+algorithmI+"}|"+"{"+traceInvocation+"}");
