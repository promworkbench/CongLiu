 ...
 *
 */
public class VisualizeVisitorPatternSet {
	//the pattern instance to dot mapping
		...
		@Plugin(name = "Visualize Visitor Pattern Instances", 
				...
				@PluginVariant(variantLabel = "Visualize Visitor Pattern Instances", 
						requiredParameterLabels = {0})// it needs one input parameter
		
		
		public JComponent visualizeTop(PluginContext context, VisitorPatternSet patternSet)
		{
			...
			{
				Dot dot = convertInstance2DOT((VisitorPatternClass)instance);
		...
		public static Dot convertInstance2DOT(VisitorPatternClass pattern)
		{
			Dot dot = new Dot();
			dot.setDirection(GraphDirection.leftRight);

			// prepare the table
			String element= "Element Class: "+pattern.getElement();
			String visitor ="Visitor Class: "+pattern.getVisitor();
			String accept= "Accept Method: "+pattern.getAccept();
			String visit = "Visit Method: "+pattern.getVisit();
			
			
			String traceInvocation= "Trace/Invocation Count: "+pattern.getTraceNumber()+"/"+pattern.getInvocationNumber();
			
			
			DotNode tab = dot.addNode("{"+element+"}|"+"{"+visitor+"}|"+"{"+accept+"}|"+"{"+visit+"}|"+"{"+traceInvocation+"}");
